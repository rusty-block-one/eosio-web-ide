...
struct reaction_type {
    static constexpr reaction_type_type thumbs_up {0};
    static constexpr reaction_type_type thumbs_down {1};
    static constexpr reaction_type_type meh {2};
};
...
struct reaction_tally {
    ...
    uint64_t meh {0};
    reaction_tally& operator+=(reaction_type_type reaction) {
        switch(reaction) {
...
struct [[eosio::table("message"), eosio::contract("talk")]] message {
    ...
    eosio::name user     = {};
    std::string content  = {};
    reaction_tally stats = {}; // reactions to the content
...
class talk : eosio::contract {
  private:
    ...
    //  @return N/A
    void record_reaction(uint64_t talk, reaction_type_type reaction) {
    ...
    //  @return N/A
    void process_reaction(eosio::name user,  uint64_t reply_to, reaction_type_type reaction) {
        //  check user
        require_auth(user);
  ...
  public:
    ...
    void post(uint64_t id, uint64_t reply_to, eosio::name user, const std::string& content) {
        ...
        // Record the message
        table.emplace(_self, [&](auto& message) {
            eosio::print("emplace id=", id);
            message.id       = id;
